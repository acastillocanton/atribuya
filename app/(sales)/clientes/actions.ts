...
export async function createClientRecord(
  ...
  // requireOrgContext also validates that the user is authenticated; it
  // throws if not, which we surface as a soft error.
  let orgCtx;
  try {
...
export async function deleteClientRecord(
  ...
  if (!orgCtx) return { ok: false, error: "No autenticado." };
