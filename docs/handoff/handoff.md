...
- **Vía B — OAuth Business Profile ❌ RECHAZADA (2026-06-24) — corrección en curso**. Trae **todas** las reseñas, pero exige que el cliente sea admin de su ficha y conecte su cuenta (OAuth). Ya configurado: consent screen modo **Testing** (External, test user `a.castillo.esv@gmail.com`), scope `business.manage`, OAuth client "Web application" (redirect `https://www.atribuya.com/api/google/oauth/callback` + apex + localhost), APIs **Account Management** + **Business Information** habilitadas. **Tras aprobación**: subir `GOOGLE_CLIENT_ID`/`SECRET`/`REDIRECT_URI` a Vercel + redeploy + probar OAuth. ⚠️ **Usar el `REDIRECT_URI` SIN www** (`https://atribuya.com/api/google/oauth/callback`): desde 2026-06-07 se invirtió el dominio canónico a apex (`www` hace 308 → `atribuya.com`), y los callbacks OAuth no siguen redirecciones. El apex ya estaba autorizado en el cliente OAuth.
  ...
  >
  > **✅ Verificación de marca APROBADA (2026-07-30, misma sesión).** Resultó inmediata, sin revisión de semanas ni vídeo demo. Lo hecho y lo aprendido:
  > - **Cláusula de Limited Use en la privacidad** (ES+EN, desplegada y verificada live): §9 ampliado con 9.1 datos accedidos vía `business.manage`, 9.2 compromiso con la Google API Services User Data Policy (sin venta, sin publicidad, sin lectura humana, sin entrenar IA) y 9.3 revocación (desconectar ficha / myaccount.google.com/permissions). Vale la pena mantenerla aunque no fuera exigida: es la que corresponde al producto.
  > - **Branding del consent screen migrado a `atribuya.com`**: nombre «Atribuya», logo subido, homepage `https://atribuya.com`, privacidad `/privacidad`, términos `/terminos`, email de soporte `alejandro@atribuya.com`; dominios autorizados `atribuya.com` + `castillocanton.com`. Al guardar, la marca quedó **verificada al momento** («Se verificó la información de tu marca y se muestra a los usuarios»).
  > - **Hallazgo clave**: `business.manage` está clasificado por Google como permiso **NO sensible** (consola → Acceso a los datos → «Tus permisos no sensibles», junto a `openid` y `email`, los 3 ya registrados). El Centro de verificación lo confirma: «Data access status: no se requiere la verificación porque tu app no solicita permisos sensibles ni restringidos». La puerta de control de Business Profile es la **aprobación de la API por proyecto** (las 3 solicitudes de junio-julio), no la verificación de scopes OAuth. En consecuencia **tampoco aplica el tope de 100 usuarios** (solo para apps sin verificar con scopes sensibles).
  > - **Verificado en la práctica** (incógnito → Conectar Google): pantalla de consentimiento limpia con logo y nombre Atribuya, sin interstitial de «app no verificada». Los avisos que se veían antes eran por el modo Testing y por la marca sin verificar, no por el scope.
  >
...
### 25.8 Fix: banner de cookies + GA faltaban en /como-funciona y /recursos (commit `51e73ae`)
Investigando por qué el evento GA4 `download_lead_magnet` no aparecía, se encontró que `isPublicPath()` en `components/analytics/Analytics.tsx` **no incluía `/como-funciona` ni `/recursos`** (sí estaban en los 3 puntos SEO, pero se olvidaron aquí — es en la práctica un **4º punto de alta** de una ruta pública). Consecuencia en esas páginas: `Analytics` devolvía `null` → no corría `CookieConsent.run()` (**sin banner**) → `gtag` nunca cargaba → `trackEvent("download_lead_magnet")` era un no-op → el evento **no podía dispararse jamás**. Fix: añadidos ambos prefijos. `/en/how-it-works` ya lo cubría el prefijo `/en/`; no hay recurso EN.
- **Verificado E2E en prod con Playwright** (spoofeando `navigator.webdriver` por el `hideFromBots`): auditada la lista COMPLETA de rutas → el banner sale en todas las públicas (marketing+legales ES/EN) y NO en la app interna (`/dashboard`, `/panel`, `/ayuda` en `(profile)`, etc. → correcto, no se rastrea). Y el flujo completo del lead magnet funciona: banner → aceptar → enviar → `gtag('event','download_lead_magnet',{magnet,locale})` llega a `region1.google-analytics.com` (hit `en=download_lead_magnet&ep.magnet=…&ep.locale=es`). **El evento se batchea**: no es instantáneo, se envía con retardo o al ocultar/navegar la pestaña (`sendBeacon` en `visibilitychange`/`pagehide`). Por eso «no salía» al mirar Tiempo real justo tras enviar sin navegar.
- ⚠️ **Limpieza pendiente**: el test E2E creó **3 leads de prueba** en `leads` con email `claude-test+magnet@atribuya.com` (+ 3 emails Brevo). Borrar vía Management API cuando se quiera.

### 25.7 Pendiente para la próxima
Artículos **#3-#12** del backlog. **Difusión LinkedIn.** **`download_lead_magnet`**: ya **verificado que se dispara y llega a GA4** (E2E, ver §25.8); queda **marcarlo como evento clave en GA4** (como `generate_lead`). Borrar los 3 leads de prueba (§25.8). Verificar **licencia de las ilustraciones** (Freepik). Reenviar sitemap en Search Console.

---

## 26. Artículo #3 (políticas de Google) + FAQPage + calendario editorial (2026-07-14). Commits `1d93a6c` → `495da33`

### 26.1 Artículo #3 del backlog (Pilar D — Cumplimiento)
Publicado **«Cómo pedir reseñas de Google sin saltarte las políticas»** (`/blog/pedir-resenas-google-sin-infringir-politicas`, `scripts/seed-post-politicas-google.mjs`) + espejo EN **«How to ask for Google reviews without breaking the rules»** (`/en/blog/ask-for-google-reviews-without-breaking-policies`, `scripts/seed-post-politicas-google-en.mjs`). Pilar D, fase Consciencia (TOFU), **personas Laura (Reputación/CX) + Carlos (CEO)** — rompe la objeción «Google penaliza todo» / «pedir reseñas manipula». Categoría nueva **«Cumplimiento y políticas de Google»** (ES) / **«Compliance & Google policies»** (EN). Estructura: «En corto», por qué importa, las 4 prohibiciones (incentivos con **cita verbatim verificada** de `support.google.com/business/answer/3474122` + review gating + contenido falso + mención al comercial), lo que sí puedes hacer, cómo cumplir y aun así atribuir, FAQ (5), resumen + CTA `/demo`. Solo la cita de incentivos es verbatim; el resto se describe sin comillas apuntando a la política. **CTA/lead magnet**: la «Guía de políticas» del backlog no existe → usa la Plantilla Excel + `/demo`. **Portada**: sembrada con placeholder `enlace-qr.png`; el usuario la cambió por una **WebP propia** (`image-aa4f…-700x394-webp`, subida en el Studio) en ES y EN. hreflang enlazado en `link-post-translations.mjs` (4º par). Borrador fuente en `Plan de contenidos/borradores/03-*.md` (gitignored).

### 26.2 Gotcha: cambio de portada quedó en borrador sin publicar (EN)
El usuario subió la portada WebP del post EN en el Studio pero **no pulsó Publish** → quedó en `drafts.post.pedir-resenas-…-en` mientras el publicado seguía con el PNG placeholder. En Sanity, editar crea un draft; hay que **Publish** para que salga a prod. Resuelto publicando el draft por API (getDocument del draft → `createOrReplace` sin prefijo `drafts.` → `delete` del draft). El ES sí estaba publicado bien. **Regla**: al cambiar algo en el Studio, comprobar que el botón Publish no quede en azul (cambios sin publicar).

### 26.3 FAQPage JSON-LD en todo el blog (commit `495da33`)
Nuevo `lib/blog/faq.ts` (`extractFaq(body)`): localiza el H2 «Preguntas frecuentes»/«Frequently asked questions» (normalizado, sin acentos) y empareja cada H3 (pregunta) con sus párrafos siguientes (respuesta) hasta el siguiente H2; devuelve `[]` si hay <2 pares. Cableado en `components/blog/BlogPostPage.tsx`: se añade un tercer bloque `FAQPage` al array de JSON-LD (junto a `BlogPosting` + `BreadcrumbList`) cuando hay FAQ. **Aplica a todo el blog sin tocar contenido**: verificado en prod #1 (4), #2 (4), #3 ES/EN (5); el post core no tiene FAQ (0, correcto). Auditoría previa confirmó que el `BlogPosting` ya trae el grafo E-E-A-T completo (author→Person `@id`, publisher Organization, mainEntityOfPage, articleSection) + la `image` resuelve a la WebP tras redeploy.

### 26.4 Leads de prueba borrados
Los «3 leads `claude-test+magnet@atribuya.com`» de §25.8 en realidad eran **2 filas con los emails reales del usuario** (`a.castillo.esv@gmail.com` + `alejandro.castillo@inseryal.es`, source `lead-magnet:plantilla-atribucion-resenas`). Borradas vía PostgREST con la service-role key (`.eq id in (...)`). Tabla `leads` vacía. (El `claude-test+…` nunca llegó a la tabla o ya se había borrado.)

### 26.5 Calendario editorial en Excel = fuente de verdad única (gitignored)
Creado `Plan de contenidos/calendario-editorial-atribuya.xlsx` (openpyxl, 4 hojas: **Calendario** maestro con estado/fecha/pilar/fase/persona/keyword/CTA/ES/EN/slug/LinkedIn + resumen con `COUNTIF`; **Personas** ref de los 4 buyer personas; **Pilares** A-E+promo; **Lead magnets** qué existe vs por crear). **Decisión**: el Excel es el dueño del calendario/estado; el `.md` de estrategia (`plan-de-contenidos.md`) **ya no duplica** la tabla del Paso 6 (solo un puntero al xlsx) — evita la deriva de dos copias (que ya había ocurrido: el backlog del `.md` no coincidía con lo publicado). Ambos ficheros gitignored (carpeta `Plan de contenidos/`). ⚠️ **LibreOffice no está instalado** → `recalc.py` falló; las fórmulas del resumen las calcula Excel/Numbers al abrir.

### 26.6 LinkedIn: etiquetar persona ANTES de escribir
Al redactar el post de LinkedIn del artículo #3 salió en voz de Javier (atribución de comerciales), cuando el artículo va a Laura+Carlos. Reescrito para **Laura** (eje: proteger la reputación, sin manipular; rompe «pedir reseñas manipula»; saca la atribución de comerciales del cuerpo). **Regla durable**: cada post de LinkedIn se etiqueta con su persona objetivo (columna del calendario) antes de escribirlo. El borrador del post de #3 para Laura está redactado pero **sin publicar en LinkedIn**.

### 26.7 Pendiente para la próxima
Artículos **#4-#12** del backlog (el #4 es «Atribuya vs Birdeye vs Excel», persona Carlos; validar keywords antes). **Difusión LinkedIn** (post de #3 para Laura listo, sin publicar; falta la serie «divide y multiplica»). **Crear los lead magnets que faltan** (checklist, plantillas de respuesta, guía de políticas, guía RGPD — solo existe la Plantilla Excel). Verificar **licencia de las ilustraciones** (Freepik). **OAuth Vía B**: sigue pendiente la verificación E2E manual en `/fichas` + publicar el consent screen (ver §7.3).

## 27. Artículo #6 (reseñas que no aparecen) + renumeración del calendario (2026-08-01)

### 27.1 Origen: post de LinkedIn del usuario sobre incentivos y review gating
El usuario publicó en su perfil de LinkedIn (2026-08-01) un post narrativo sobre incentivos por reseñas y review gating (la anécdota del postre gratis). Análisis: NO se escribe un artículo de «incentivos y review gating» porque el #3 ya lo cubre (canibalización); el ángulo diferencial del post es la **penalización silenciosa** (Google filtra/borra reseñas sin avisar) → se convierte en el artículo #6, insertado como siguiente pieza del calendario. Mientras el #6 no existía, el enlace en comentarios del post de LinkedIn debía apuntar al #3; ahora puede apuntar al #6.

### 27.2 Artículo #6 (Pilar D — Cumplimiento, TOFU, personas Laura + Carlos)
Publicado **«Por qué tus reseñas de Google no aparecen (y cómo saber si te están filtrando)»** (`/blog/resenas-google-no-aparecen`, `scripts/seed-post-resenas-no-aparecen.mjs`) + espejo EN **«Why your Google reviews are not showing up (and how to tell if they are being filtered)»** (`/en/blog/google-reviews-not-showing-up`, `scripts/seed-post-resenas-no-aparecen-en.mjs`, portada reutilizada del asset ES sin re-subir). Keyword «mis reseñas de Google no aparecen» (el título/H1 filtra hacia intención de negocio, la keyword gorda es mayoritariamente de consumidor). Categorías **reutilizadas** del #3 (`category.cumplimiento-politicas-google` / `category.compliance-google-policies`). Estructura GEO: «En corto», descartar el retraso normal, la eliminación silenciosa con **estadística con fuente** (292M de reseñas retiradas en 2025, informe oficial de Google en blog.google de abril 2026, verificado con WebFetch), 6 causas frecuentes, diagnóstico en 6 pasos, plan de recuperación, sección Atribuya (registro pedidas vs publicadas), FAQ (5), resumen + CTA `/demo`. Sin citas verbatim de la ayuda de Google (se describe + enlaza, regla de §24): `support.google.com/business/answer/10313341` (reseñas que faltan o se han retrasado) + `contributionpolicy/answer/7400114`. **Portada**: placeholder `mis-resenas.png` (panel de reseñas del comercial); el usuario probablemente la cambie por WebP propia en el Studio (⚠️ `--force` del seed ES la pisaría). hreflang: **7º par** en `link-post-translations.mjs`, ejecutado. Verificado E2E en prod: 200 en ambas URLs, hreflang es/en/x-default correcto, canonical, FAQPage + BlogPosting con la Person canónica `#person`, enlaces internos (#3, core, plantilla, /demo).

### 27.3 Calendario: nuevo #6 insertado, antiguos #6-#12 → #7-#13
En `calendario-editorial-atribuya.xlsx` (fila 11): el nuevo #6 se insertó ANTES de «Responder reseñas de Google» porque ese artículo (ahora #7, 2026-08-25) usa como CTA las plantillas de respuesta, lead magnet aún por crear. Fórmulas del resumen ampliadas a fila 18 (14 piezas, 7 publicadas tras el #6). Nota: LibreOffice SÍ está instalado (`/Applications/LibreOffice.app/Contents/MacOS/soffice`, hay que anteponerlo al PATH para `recalc.py`); corrige lo dicho en §26.5.

### 27.4 Pendiente para la próxima
Artículos **#7-#13** (el #7 requiere crear antes el pack de plantillas de respuesta). Post de LinkedIn del núcleo. Actualizar el enlace en comentarios del post de LinkedIn de incentivos/gating para que apunte al #6. Marcar el par ES/EN en el Studio si algún seed se relanza con `--force` (recorrer `link-post-translations.mjs`).

## 28. Artículo #7 (responder reseñas) + calendario acelerado por los posts de LinkedIn (2026-08-02)

### 28.1 Contexto: 6 posts personales de LinkedIn marcan el ritmo
El usuario trae 6 posts personales escritos (1 incentivos/gating publicado 2026-08-01, 2 tablet programado, 3 Omnibus, 4 mito del 5,0, 5 reseñas falsas, 6 GEO/IA). Decisión: los artículos del blog dejan de marcar el ritmo (quincenal) y pasan a servir a los posts (semanal en agosto), porque los artículos los produce Claude en el día y los posts son el activo escaso. Mapeo completo en la **hoja nueva «LinkedIn personal»** del calendario xlsx (post → gancho → persona → artículo destino → semana → estado). Regla: artículo publicado unos días antes del post; enlace en el primer comentario (o en el cuerpo tras la P.D. si el post va programado, porque LinkedIn no permite programar comentarios).

### 28.2 Calendario reestructurado
`calendario-editorial-atribuya.xlsx`: #7 adelantado (2026-08-10→publicado 2026-08-02); **3 artículos nuevos** insertados desde los posts de LinkedIn: **#8 «Directiva Omnibus y reseñas: qué tienes que poder demostrar»** (Pilar D, Carlos+Laura, 17-ago; ⚠️ exige verificar la directiva 2019/2161 + transposición española antes de escribir; NO existe mención previa a Omnibus en ningún doc del plan, se comprobó), **#9 «Reseñas falsas en Google: cómo denunciarlas y protegerte»** (Pilar C, Laura+Carlos, 24-ago), **#10 «Reseñas y visibilidad en IA»** (Pilar A, Javier+Carlos, 31-ago; ojo con la frontera de política en el truco de «pregunta bien»). Los antiguos #8-#13 pasan a **#11-#16** con fechas quincenales desde el 14-sept. Fórmulas del resumen ampliadas a fila 21 (17 piezas).

### 28.3 Artículo #7 (Pilar C — Reputación local, personas Marta+Laura)
Publicado **«Cómo responder reseñas de Google: guía con plantillas listas para adaptar»** (`/blog/responder-resenas-google`, `scripts/seed-post-responder-resenas.mjs`) + espejo EN (`/en/blog/respond-to-google-reviews`, `-en.mjs`, portada reutilizada). Categoría **nueva** «Reputación local» (`category.reputacion-local`) / «Local reputation» (`category.local-reputation`). Estructura: «En corto», datos (LCRS 2026 **verificada en fuente primaria vía WebFetch**: 80% usaría negocio que responde a todo, 42% descarta a quien no responde, 50% rechaza respuestas de plantilla genérica), 4 principios (escribir para el próximo cliente; rapidez; concreción; confidencialidad, con nota sectorial de clínicas: no confirmar condición de paciente), **7 plantillas inline en blockquotes** (3 positivas + 4 negativas, incl. la neutra para no-clientes que enlaza con la trazabilidad), prohibiciones (no ofrecer nada por retirar/cambiar reseña), organización en equipo (alertas 1-2★ + contexto vía atribución), FAQ (5), resumen + CTA `/demo`. `PortableTextComponents` ya soportaba blockquote (verificado antes de publicar). hreflang **8º par**. Verificado E2E en prod (hreflang, FAQPage 5, Person canónica, 7 blockquotes renderizados). **Pendiente**: el pack de plantillas de respuesta **descargable** (lead magnet del Pilar C) sigue por crear; el artículo sale con las plantillas inline y CTA a demo, patrón «interim» como el #3.
...
## 30. Segundo carril de LinkedIn: piezas ligeras + plan A3 (2026-08-02, cierre de sesión)

Cadencia de LinkedIn ampliada a **1 post grande + 2 piezas ligeras por semana** (troceo «divide y multiplica» del plan). Redactados los **10 borradores de piezas ligeras** de agosto (2 por semana, en la voz canalla del usuario, derivados de los artículos/posts de cada semana) y montado un **plan imprimible A3 apaisado** (1 página, marca + contacto, 5 columnas semanales con artículo/post grande/2 ligeras completas + reglas en el pie). Generador y PDF en `Plan de contenidos/linkedin/` (gitignored; `build.py` con Playwright regenera el PDF). **Enviado por email** a `a.castillo.esv@gmail.com` vía Brevo SMTP el 2026-08-02 (gotcha: `BREVO_FROM_EMAIL` ya viene con formato `Nombre <email>`, pasarlo tal cual a nodemailer, no envolverlo en `{name, address}` → «451 Invalid from»). Hoja «LinkedIn personal» del calendario xlsx con las filas L1-L10. **Misma sesión**: fix del gotcha de drafts de Sanity en el #7 (portadas del usuario publicadas por API, ES+EN, hreflang conservado) y 2 informes de GSC verificados como falsos positivos (noindex de `www/en` = rastreo antiguo pre-fix; «página con redirección» de las 3 variantes no canónicas = comportamiento correcto, su validación siempre saldrá en error y no hay que validarla).

**Pendiente próxima sesión**: artículo #8 (Directiva Omnibus, verificar directiva 2019/2161 + transposición española en fuentes primarias antes de escribir; semana del 17), luego #9 (falsas) y #10 (GEO). Opcional ofrecido al usuario: revalidación bajo demanda vía webhook de Sanity → `/api/revalidate` (acabaría con las esperas de ISR al publicar en el Studio); sin respuesta aún.
