...
- **Iubenda (EN CURSO)**: sustituido el banner propio (`CookieBanner`+`lib/consent.ts` borrados) por la Cookie Solution de Iubenda. Historia del enredo: primero se probó Google Consent Mode v2, pero el plan es **Gratis** (no incluye Consent Mode) y su embed es el **widget** (`embeds.iubenda.com/widgets/<uuid>.js`), que **no arranca con `next/script`** porque depende de `document.currentScript` (null en carga async) → solo salía el icono flotante, no el banner. Solución final = **embed clásico** en `Analytics.tsx`: un único script inline que fija `window._iub.csConfiguration` (`siteId 4595912`, `cookiePolicyId 51557107`, `lang` por locale, `perPurposeConsent`, banner con rechazar) y **luego** inyecta `cs.iubenda.com/autoblocking/4595912.js` + `iubenda_cs.js` en el mismo hilo (orden determinista, commit `959ffb4`). Modelo = **autobloqueo** (GA bloqueado hasta aceptar). `CookiePrefs`→`_iub.cs.api.openPreferences()`. CSP con dominios Iubenda + `frame-src`. Privacidad §8 (ES+EN) reescrita. Env en Vercel (3 entornos) + `.env.local`. Cookie policy: `iubenda.com/privacy-policy/51557107/cookie-policy`. **PENDIENTE**: verificar en navegador (incógnito nuevo) que el banner **autoarranca** tras el fix `959ffb4`; si solo sale el icono flotante, **completar el «Paso 2 · configurar» del dashboard de Iubenda** (estaba a 1/3, login `a.castillo.esv@gmail.com`) para publicar el aviso como banner. El usuario preguntó si GA ya lee: GA está **autobloqueado** por Iubenda hasta el consentimiento (por diseño), así que no medirá hasta aceptar.

**Estado para la próxima sesión**: web y blog en buen estado (Lighthouse verde). **Acción pendiente inmediata**: confirmar que el banner de Iubenda autoarranca en incógnito (y, si no, rematar el Paso 2 del dashboard de Iubenda). Bloqueante técnico del producto sigue siendo **Google OAuth Vía B** (esperando aprobación, ver §7.3). Pendientes menores de marketing: testimonio/logos reales y los de la revisión buyer persona (§22).
