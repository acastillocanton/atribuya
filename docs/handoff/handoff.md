...
- **Vía B — OAuth Business Profile ⏳ ESPERANDO A GOOGLE**. Trae **todas** las reseñas, pero exige que el cliente sea admin de su ficha y conecte su cuenta (OAuth). Ya configurado: consent screen modo **Testing** (External, test user `a.castillo.esv@gmail.com`), scope `business.manage`, OAuth client "Web application" (redirect `https://www.atribuya.com/api/google/oauth/callback` + apex + localhost), APIs **Account Management** + **Business Information** habilitadas. **Falta solo la aprobación de Google**: se envió la solicitud de acceso a las Business Profile APIs (desbloquea la Reviews API v4) el 2026-06-07 → caso de soporte **`7-4031000041620`**, revisión 7-10 días hábiles. Hasta que aprueben, la v4 da `PERMISSION_DENIED` y cubre el hueco la Vía A. **Tras aprobación**: subir `GOOGLE_CLIENT_ID`/`SECRET`/`REDIRECT_URI` a Vercel + redeploy + probar OAuth. ⚠️ **Usar el `REDIRECT_URI` SIN www** (`https://atribuya.com/api/google/oauth/callback`): desde 2026-06-07 se invirtió el dominio canónico a apex (Opción A SEO: `www` hace 308 → `atribuya.com`), y los callbacks OAuth no siguen redirecciones. El apex ya estaba autorizado en el cliente OAuth. **Recordatorio remoto vigente** (reemplaza al del 18-jun): `trig_018GQgLgMKyzj6SpJztSqfAK` (25-jun, revisar cuota + activar Vía B).
  ...
  >
  > **Actualización (2026-06-11) — re-solicitud ENVIADA.** Plan completado: (1) buzón ImprovMX ✅ (verificado E2E el 10-jun; MX `mx1/mx2.improvmx.com` confirmados); (2) cuenta de Google `alejandro@atribuya.com` creada ✅; (3) añadida al perfil GBP de **Castillo Cantón** (decisión del 11-jun; descartado Mon Petit) ✅; (4)-(5) cuenta de organización + IAM ✅; (6) **formulario reenviado el 2026-06-11** desde `alejandro@atribuya.com`, con número de proyecto `443155173600`, web `https://atribuya.com` y caso de uso de solo lectura (atribución de reseñas, OAuth por cliente, scope `business.manage`). No tenemos número de caso nuevo registrado; si llega email de confirmación, anotarlo aquí. **Seguimiento**: vigilar la cuota "Requests per minute" de Business Information API (0 = pendiente, ~300 = aprobado); **verificada en 0 el 2026-06-21 → sigue pendiente**. Recordatorio remoto reprogramado al 25-jun (`trig_018GQgLgMKyzj6SpJztSqfAK`, final de la ventana estimada de Google). **SPF resuelto el 2026-06-21**: añadido el TXT en Hostinger (`v=spf1 include:spf.brevo.com include:spf.improvmx.com ~all`, único registro SPF con ambos includes).
...
En orden: ~~Brevo (§7.2)~~ ✅ → ~~Google Places Vía A (§7.3)~~ ✅ → ~~DPA (§7.6)~~ ✅ → **solo queda Google OAuth Vía B (§7.3)**, re-solicitud enviada el 2026-06-11 con `alejandro@atribuya.com` (~7-10 días hábiles, ≈ 19-25 jun). Lo demás (pricing, setup, billing) son decisiones de negocio (§8), no técnicas. **El camino crítico está técnicamente cerrado salvo la aprobación de Google, que no depende de nosotros.**
