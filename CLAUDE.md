...
| Google Cloud — OAuth Business Profile (Vía B) | ⏳ Pendiente | Más largo: consent screen + OAuth client + solicitud de cuota de la Reviews API v4 (aprobación manual de Google, días/semanas). Vía A (Places) cubre el hueco mientras tanto. Redirect URI a registrar: `https://www.atribuya.com/api/google/oauth/callback` (atribuya.com hace 308→www) + `http://localhost:3000/...` para dev. Scopes: `openid email https://www.googleapis.com/auth/business.manage`. Vars: `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`GOOGLE_OAUTH_REDIRECT_URI`. |
...
En orden: ~~Brevo SMTP~~ (✅ hecho 2026-06-06) → ~~Google Cloud Places (Vía A)~~ (✅ hecho 2026-06-07) → **OAuth Business Profile (Vía B) → DPA**. Ver §7 para el detalle de cada uno. Lo demás de §8 son decisiones de negocio, no bloqueantes técnicos.
