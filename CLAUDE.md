...
| —. **Plan de contenidos + lead magnet + 1er artículo del Pilar A (2026-07-05)**: motor de captación para leads/ventas siguiendo el marco de 8 pasos de la guía BIG school (`Plan de contenidos/`, gitignored). **Estrategia** (detalle → handoff §24): keywords de nicho de bajo volumen + head-terms de intención equivocada → apuesta por autoridad temática + long-tail agregado + **GEO** (motores de respuesta) + LinkedIn + lead magnet (fuentes primarias: guía IA de Google + paper GEO KDD 2024 + BrightLocal). **Artículo #1** publicado en Sanity: `/blog/conseguir-resenas-google-equipo-comercial` (`scripts/seed-post-conseguir-resenas.mjs`, categoría nueva «Conseguir reseñas de Google»), optimizado SEO/GEO con cita verbatim verificada de la política de Google + estadística con fuente + FAQ, cover `enlace-qr`. **Lead magnet** «Plantilla de atribución de reseñas» (Excel en `public/recursos/` + página indexable `/recursos/plantilla-atribucion-resenas` con `FAQPage` JSON-LD + captación `submitLeadMagnet`→`leads` `source:lead-magnet:*` + evento GA4 `download_lead_magnet` + entrega Brevo). **Fix JSON-LD** (todos los posts): `publisher` autocontenido + `articleSection`. Misma sesión (tarde): auditoría de **meta descriptions** (5 recortadas a ≤160; títulos todos OK), auditoría **alt/rel** (todo correcto; `title` masivo descartado por anti-patrón SEO/a11y), e **ilustraciones editoriales WebP transparentes** en `/como-funciona` y `/precios` (ES+EN; prop opcional `illustration` en `PricingSection`, la home no cambia). Pendiente: artículos #2-#12, difusión LinkedIn, marcar `download_lead_magnet` como evento clave en GA4, verificar licencia de las ilustraciones (Freepik) | ✅ 2026-07-05 |
| —. **Artículo #2 + blog bilingüe completo + hreflang (2026-07-06)**: publicado el **artículo #2** (Pilar B) «Ranking de reseñas por comercial sin generar rivalidad» (`/blog/ranking-resenas-comercial-sin-rivalidad`, cat. nueva «Equipo comercial»). El usuario cambió las **portadas** de los 3 posts ES por ilustraciones WebP propias desde el Studio (assets de Sanity, ⚠️ un `--force` de los seed ES las pisaría). **Traducciones EN** de los 2 posts que faltaban (conseguir reseñas → `get-google-reviews-with-your-sales-team`, cat. «Getting Google reviews»; ranking → `sales-rep-review-ranking-without-rivalry`, cat. «Sales team»), reutilizando la portada WebP del post ES sin re-subir → **los 3 artículos ya tienen espejo ES+EN**. **hreflang**: campo nuevo `translationSlug` en el schema de `post` + proyección en query + `alternates.languages` (es/en/x-default→ES) en `[slug]`/`en/[slug]` + backfill `scripts/link-post-translations.mjs` (patch idempotente por pares). ⚠️ **Mantenimiento**: al traducir, añadir el par en `link-post-translations.mjs` y correrlo (o rellenar `translationSlug` en el Studio en ambos); un seed con `--force` borra el campo → recorrer el script. Un solo `sitemap.xml` (ES+EN) es lo correcto, no se hacen sitemaps por idioma. **Fix (mismo día, `51e73ae`)**: `isPublicPath()` en `components/analytics/Analytics.tsx` no incluía `/como-funciona` ni `/recursos` → en esas páginas no salía el banner de cookies ni cargaba GA, así que `download_lead_magnet` no podía dispararse. Añadidos ambos prefijos (`isPublicPath` es en la práctica un **4º punto de alta** de ruta pública, tras los 3 SEO). Verificado E2E en prod con Playwright: banner en todas las públicas, no en la app interna; el evento `download_lead_magnet` se dispara y **llega a GA4** (se batchea, no es instantáneo). *(Leads de prueba del lead magnet borrados el 2026-07-14: en realidad eran 2 filas con los emails reales de Alejandro `a.castillo.esv@gmail.com` + `alejandro.castillo@inseryal.es`, no el `claude-test+magnet@…` que anotamos; tabla `leads` vacía tras el borrado vía PostgREST con service-role.)* *(El evento `download_lead_magnet` se marcó como evento clave en GA4 el 2026-07-07.)* Detalle → handoff §25 | ✅ 2026-07-06 |
| —. **Artículo #3 (Pilar D) + FAQPage en todo el blog + calendario editorial (2026-07-14)**: publicado el **artículo #3** «Cómo pedir reseñas de Google sin saltarte las políticas» (`/blog/pedir-resenas-google-sin-infringir-politicas`, seed `seed-post-politicas-google{,-en}.mjs`, cat. nueva «Cumplimiento y políticas de Google» / «Compliance & Google policies», personas **Laura+Carlos**, TOFU) + espejo EN + hreflang (4º par en `link-post-translations.mjs`). Portada WebP propia del usuario (subida en el Studio; **gotcha**: el cambio EN quedó en borrador sin publicar → se publicó por API). **`FAQPage` JSON-LD en toda la plantilla del blog** (`lib/blog/faq.ts` + `BlogPostPage.tsx`): extrae la sección de FAQ del cuerpo y emite el rich result con ≥2 pares (aplica a #1/#2/#3; el core no tiene FAQ). Borrados los **leads de prueba** (2 filas, emails reales del usuario). **Calendario editorial** ahora en `Plan de contenidos/calendario-editorial-atribuya.xlsx` (gitignored, **fuente de verdad única** del backlog/estado; el `.md` deja de duplicar la tabla). Regla nueva: cada pieza y cada post de LinkedIn se **etiqueta con su buyer persona** antes de escribir. Detalle → handoff §26 | ✅ 2026-07-14 |
| —. **Artículo #4 (comparativa)**: «Atribuya vs Birdeye vs Excel: qué resuelve cada uno» publicado ES (`/blog/atribuya-vs-birdeye-vs-excel`) + espejo EN, cat. nueva «Comparativas», seeds `seed-post-atribuya-vs-birdeye{,-en}.mjs` + 5º par de hreflang en `link-post-translations.mjs`. Cubre además el pendiente de comparativa de la revisión por buyer persona (como artículo; la mini-tabla en `/precios`·`/casos` sigue abierta) | ✅ 2026-07-30 |
| —. **Artículo #5 (Pilar B)**: «Incentivos y bonos comerciales basados en reseñas de Google» publicado ES (`/blog/incentivos-comerciales-resenas-google`) + espejo EN (`/en/blog/sales-rep-incentives-google-reviews`), persona Javier, cat. «Equipo comercial»/«Sales team» reutilizadas, seeds `seed-post-incentivos-comerciales{,-en}.mjs` + 6º par de hreflang. Verificado live (hreflang + FAQPage). Portada WebP propia subida por el usuario en el Studio (ES+EN; ⚠️ un `--force` del seed ES la pisaría). Misma sesión: **fix del selector de idioma** (`useAltLangHref` lee el hreflang del `<head>` → en artículos lleva al post traducido exacto, no al índice) y **fix E-E-A-T** (los posts EN emitían una Person con `@id` propio → ahora ES+EN comparten el `@id` canónico `/blog/autor/{slug}#person`; auditoría JSON-LD 12/12 OK en prod) | ✅ 2026-07-30 |
| —. **Artículo #6 (Pilar D)**: «Por qué tus reseñas de Google no aparecen (y cómo saber si te están filtrando)» publicado ES (`/blog/resenas-google-no-aparecen`) + espejo EN (`/en/blog/google-reviews-not-showing-up`), personas Laura+Carlos, cat. «Cumplimiento y políticas de Google» reutilizada del #3, seeds `seed-post-resenas-no-aparecen{,-en}.mjs` + 7º par de hreflang. Ángulo: la penalización silenciosa (Google filtra sin avisar; estadística 292M reseñas retiradas en 2025, blog oficial de Google) + diagnóstico en 6 pasos; nace del post de LinkedIn del usuario sobre incentivos/gating (2026-08-01) sin canibalizar el #3. Verificado E2E en prod (hreflang, FAQPage, Person canónica). Calendario: el nuevo #6 se insertó como siguiente pieza y los antiguos #6-#12 pasan a #7-#13 (detalle → handoff §27) | ✅ 2026-08-01 |
| —. **Artículo #7 (Pilar C) + calendario acelerado + hoja LinkedIn**: «Cómo responder reseñas de Google: guía con plantillas listas para adaptar» publicado ES (`/blog/responder-resenas-google`) + espejo EN (`/en/blog/respond-to-google-reviews`), personas Marta+Laura, cat. nueva «Reputación local»/«Local reputation», 7 plantillas de respuesta inline (blockquotes), stat verificada LCRS 2026 (80%/42%/50%), seeds `seed-post-responder-resenas{,-en}.mjs` + 8º par de hreflang. Verificado E2E en prod. **Calendario reestructurado a ritmo semanal en agosto** ligado a los 6 posts personales de LinkedIn del usuario: 3 artículos nuevos al backlog (#8 Directiva Omnibus 17-ago, #9 reseñas falsas 24-ago, #10 GEO/IA 31-ago; los antiguos #8-#13 pasan a #11-#16 desde sept) + hoja nueva «LinkedIn personal» en el xlsx con el mapeo post→artículo→persona→estado. Pack de plantillas descargable (lead magnet) pendiente. Detalle → handoff §28 | ✅ 2026-08-02 |
| —. **Pack de plantillas de respuesta (2º lead magnet)**: Word de marca con 20 plantillas (`public/recursos/plantillas-respuesta-resenas-google.docx`, portada Atribuya + contacto en pie; **regla nueva del founder: todo entregable lleva marca + contacto**, aplicada también al Excel existente vía sharedStrings para no romper validaciones) + página indexable `/recursos/plantillas-respuesta-resenas` (FAQPage, alta 3 puntos SEO) + infra de magnets generalizada (`MAGNETS` en la action, copy de email por magnet, prop `magnet` en el form, GA4 distingue por parámetro) + CTA en el artículo #7 ES+EN. Verificado E2E en prod (formulario→lead con source propio→borrado, descarga 200, CTA, sitemap). Detalle → handoff §29 | ✅ 2026-08-02 |
| —. **Runbook de onboarding de cliente** (`docs/onboarding-cliente.md`): checklist proveedor+cliente de 5 fases (DPA → org → ficha+OAuth → equipo → primer circuito de atribución → vigilancia), chuleta de gotchas y checklist imprimible. Guion del setup de 129 €. Además, el **workflow horario de GitHub Actions dispara también la Vía B** (`sync-google-reviews`): las fichas OAuth quedaban a merced del cron diario (hasta 24h de retraso). ⚠️ Limitación conocida: el botón «Buscar mis reseñas» (`/api/sync/now`) solo lanza la Vía A → en fichas conectadas no hace nada (mejora pendiente) | ✅ 2026-07-30 |
| —. Google OAuth Business Profile (Vía B) — todas las reseñas | ✅ **VERIFICADA E2E 2026-07-30**. Aprobada por Google el 2026-07-10 (proyecto `443155173600`, tras 3 solicitudes) y probada de punta a punta en prod con la org de prueba AleCris + ficha real Mon Petit Caprice: OAuth conecta, el cron trae las **7/7 reseñas** (`source='business_profile'`, más allá del top-5 de la Vía A) y la reseña de 1★ **disparó el email de alerta**. Fixes de la sesión: perfil del super_admin recreado (el wipe de orgs del 07-03 lo borró por cascade → login rebotaba con `no-profile`), gotcha del **consent granular** (hay que marcar la casilla de fichas de empresa o el token llega sin `business.manage` → instrucción de onboarding), bug `daf82fc` (`linkGoogleLocation` normaliza `google_location_resource` a `accounts/…/locations/…`; la v4 daba 404 con el nombre corto), y **API legacy «Google My Business API» habilitada** (oculta del buscador, solo por URL directa). **Consent screen PUBLICADO** (misma sesión): «En producción» sin verificar → cualquier Google Account puede conectar (aviso «app no verificada» + tope 100 usuarios); fichas reconectadas tras publicar (los refresh tokens de Testing caducaban a los 7 días). **Verificación de marca APROBADA (2026-07-30, inmediata)**: cláusula Limited Use en la privacidad (ES+EN, live), branding del consent screen migrado a `atribuya.com` (nombre, logo, legales, soporte `alejandro@atribuya.com`) y marca verificada al guardar. Hallazgo: `business.manage` es permiso **NO sensible** → sin verificación de scopes ni tope de 100 usuarios (la puerta de GBP es la aprobación de la API por proyecto). Consent limpio verificado en incógnito. **Reconciliación Vía A→B implementada** (misma sesión): el cron de la Vía B reclama las filas `places:%` pre-OAuth (`decideCrossSourceClaim`, conserva atribución) y barre las huérfanas sin atribución (`sweepPlacesLeftovers`); verificado E2E en prod. **Vía B COMPLETA, sin pendientes.** Detalle → handoff §7.3. |
...
**Estado actual de datos (verificado 2026-07-30):**
- El 2026-07-03 se **borraron las 2 orgs antiguas** (Acme Promotora y AleCris) a petición del founder (audit `org_deleted` × 2). El cascade se llevó también el perfil del super_admin (recreado el 2026-07-30 vía PostgREST: role `admin`, `org_id` null — el middleware exige fila en `profiles` para todos).
- `organizations` — 1 fila: **AleCris** (trial, plan `standard`, org de prueba recreada para la verificación E2E de la Vía B).
- `locations` — 3 fichas conectadas por **Vía B (OAuth)**: "Castillo Cantón - Consultor SEO Castellón" (0 reseñas en Google), **"Mon Petit Caprice"** (7 reseñas) y **"EnchufeSolar Valencia"** (7 reseñas).
- `reviews` — **14 filas reales** (`source='business_profile'`), sin duplicados: la transición Vía A→B queda reconciliada automáticamente por el cron.
- `profiles` — 2 filas (super_admin sin org + `alejandro@atribuya.com` admin de AleCris). `clients` y `share_links` — vacías.
...
| **Google OAuth (Vía B)** | ✅ **VERIFICADA E2E 2026-07-30**. Env en Vercel prod (apex SIN www en el redirect). Probada con ficha real: trae todas las reseñas (7/7, no el top-5) + alerta de 1★ enviada. Las 3 APIs habilitadas en el proyecto `443155173600` (Account Management, Business Information y la legacy **Google My Business API** v4 de reseñas, oculta del buscador → habilitar por URL directa). **Gotcha de onboarding**: en el consent hay que **marcar la casilla** de fichas de empresa o el token llega sin `business.manage`. **Consent screen publicado** («En producción» sin verificar, 2026-07-30): clientes externos pueden conectar con el aviso de app no verificada (tope 100 usuarios); la verificación de marca queda opcional para más adelante. Detalle → handoff §7.3. |
...
En orden: ~~Brevo SMTP~~ (✅ 2026-06-06) → ~~Google Cloud Places (Vía A)~~ (✅ 2026-06-07) → ~~DPA~~ (✅ 2026-06-07) → ~~OAuth Business Profile (Vía B)~~ (✅ **VERIFICADA E2E + consent screen publicado 2026-07-30**: 7/7 reseñas, alerta 1★, clientes externos ya pueden conectar). **Camino crítico COMPLETO: no queda ningún bloqueante técnico** para el primer cliente; lo demás de §8 son decisiones de negocio.
