...
| —. **Consentimiento: banner propio → Iubenda Cookie Solution (2026-07-05, EN CURSO)**: sustituido el banner casero (`CookieBanner`+`lib/consent.ts`, borrados) por Iubenda vía **embed clásico** (`Analytics.tsx`: `window._iub.csConfiguration` inline con `siteId 4595912`+`cookiePolicyId 51557107` + autobloqueo `cs.iubenda.com/autoblocking/4595912.js` + `iubenda_cs.js` + gtag). **Plan Gratis = autobloqueo** (GA bloqueado hasta aceptar; Google Consent Mode es de pago, descartado). **NO usar el widget** `embeds.iubenda.com/widgets/<uuid>.js`: depende de `document.currentScript` y no arranca con `next/script` async. `CookiePrefs` (footer «Cookies») → `_iub.cs.api.openPreferences()`. CSP: +`cdn/cs/embeds/idb.iubenda.com` + directiva nueva `frame-src 'self' https://*.iubenda.com`. Privacidad §8 (ES+EN) reescrita a autobloqueo + enlace a política de cookies de Iubenda. Env en Vercel (3 entornos): `NEXT_PUBLIC_IUBENDA_SITE_ID=4595912`, `NEXT_PUBLIC_IUBENDA_COOKIE_POLICY_ID=51557107`. ⚠️ **PENDIENTE DE VERIFICAR**: el icono flotante sale (Iubenda carga) pero faltaba confirmar que el **banner autoarranca** en incógnito; último fix (`959ffb4`) fija la config antes de los loaders. Si tras eso solo sale el icono → **completar el «Paso 2 · configurar» del dashboard de Iubenda** (estaba a 1/3) para que el aviso se publique como banner. Detalle → handoff | ⏳ 2026-07-05 |
...
| **Analítica (GA4)** | ✅ `G-GKYPWE3QRK`, `NEXT_PUBLIC_GA_ID` en Vercel. Solo páginas públicas/legales (la app interna NO se rastrea), hits solo en prod. Evento de conversión **`generate_lead`** en el envío del formulario (`lib/gtag.ts`), marcado como **evento clave** en GA4 el 2026-06-11. Vinculado a Search Console. **Consentimiento gestionado ahora por Iubenda** (ver fila siguiente), no por el banner propio (retirado); GA queda autobloqueado por Iubenda hasta que el usuario acepta. |
| **Consentimiento (Iubenda)** | ⏳ **EN CURSO (2026-07-05)**. Cookie Solution de Iubenda vía embed clásico (`Analytics.tsx`), plan **Gratis** = autobloqueo. `siteId 4595912`, `cookiePolicyId 51557107`; env `NEXT_PUBLIC_IUBENDA_SITE_ID`/`_COOKIE_POLICY_ID` en Vercel (3 entornos) + `.env.local`. Login del dashboard: `a.castillo.esv@gmail.com`. Cookie policy: `iubenda.com/privacy-policy/51557107/cookie-policy`. ⚠️ **Falta verificar en navegador que el banner autoarranca** (sale el icono flotante; último fix `959ffb4` fija config antes de loaders). Si solo sale el icono, completar el **Paso 2 · configurar** del dashboard (estaba a 1/3). **No usar el widget** (rompe con `next/script`, ver §3). |
...
- **🟡 pendientes de la revisión por buyer persona (2026-07-04)** — impacto medio, no bloqueantes (detalle → handoff §22):
  ...
  - ✅ **Primer artículo del blog** publicado el 2026-07-04 (ver §3 / handoff §23.1). Quedan más artículos por escribir.
