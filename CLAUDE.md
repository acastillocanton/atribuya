...
| —. Google OAuth Business Profile (Vía B) — todas las reseñas | ❌ **RECHAZADA 2026-06-24** por Google: "No approved project" + "The listing ID is associated with a different website". Causa raíz: consent screen sin URLs (homepage/privacidad/términos vacíos) + dominio autorizado `atribuya.com` ≠ web de la ficha GBP de Castillo Cantón (`castillocanton.com`). **Plan de corrección en curso** (ver §7 + handoff §7.3): (1A) rellenar consent screen con `castillocanton.com` como homepage + privacidad (`/politica-de-privacidad/`) + términos (`/terminos-de-servicio/`) + añadir `castillocanton.com` como dominio autorizado; (1B) ✅ página de términos creada (`castillocanton.com/terminos-de-servicio/`, live 2026-06-24); (2) reenviar solicitud. Pendiente solo 1A (manual en Cloud Console) + reenvío. |
...
| **Google OAuth (Vía B)** | ❌ **RECHAZADA 2026-06-24**. Motivo Google: "No approved project. The listing ID is associated with a different website." Causa raíz: el consent screen del proyecto Cloud `atribuya` tenía los campos de homepage/privacidad/términos **vacíos**, y el único dominio autorizado era `atribuya.com`, que no coincide con la web de la ficha GBP de Castillo Cantón (`castillocanton.com`). **Plan de corrección en curso** (detalle → handoff §7.3): paso 1A = rellenar consent screen con `castillocanton.com` (homepage + `/politica-de-privacidad/` + `/terminos-de-servicio/` + añadir `castillocanton.com` como dominio autorizado); paso 1B ✅ página de términos creada (`castillocanton.com/terminos-de-servicio/`, live 2026-06-24); paso 2 = reenviar solicitud. **Estado al retomar (2026-06-24)**: solo falta el paso 1A (manual en Cloud Console → Información de la marca, campos aún vacíos) + el reenvío. **Tras aprobar**: subir `GOOGLE_CLIENT_ID`/`SECRET`/`REDIRECT_URI` a Vercel + probar OAuth. ⚠️ **Usar el `REDIRECT_URI` SIN www** (`https://atribuya.com/api/google/oauth/callback`). |
...
En orden: ~~Brevo SMTP~~ (✅ hecho 2026-06-06) → ~~Google Cloud Places (Vía A)~~ (✅ hecho 2026-06-07) → ~~DPA~~ (✅ hecho 2026-06-07, ver fase 8) → **solo queda OAuth Business Profile (Vía B)**, solicitud **rechazada el 2026-06-24** (ver §7). Plan de corrección: página de términos ✅ creada (`castillocanton.com/terminos-de-servicio/`); falta alinear el consent screen con `castillocanton.com` (paso 1A, manual en Cloud Console) + reenviar. Ver §7 y handoff §7.3. Lo demás de §8 son decisiones de negocio, no bloqueantes técnicos.
