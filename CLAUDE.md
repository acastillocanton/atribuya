...
| —. **Iubenda revertido (2026-07-05)**: se deshizo la integración de Iubenda (commits `aa3204a`/`6b2d2fa`/`146c3d1`/`959ffb4`) y se **restauró el banner de consentimiento propio** (`components/analytics/CookieBanner.tsx` + `lib/consent.ts` + orquestador en `Analytics.tsx`): opt-in RGPD, gtag no carga hasta «Aceptar», revocable desde el footer (`CookiePrefs` vía `CONSENT_EVENT`), hits solo en prod. Revertidos también CSP de `next.config.ts`, Privacidad §8 (ES+EN) y `.env.example` a su estado pre-Iubenda. Env `NEXT_PUBLIC_IUBENDA_*` a eliminar de Vercel (3 entornos) | ✅ 2026-07-05 |
...
| **Analítica (GA4)** | ✅ `G-GKYPWE3QRK`, `NEXT_PUBLIC_GA_ID` en Vercel. Solo páginas públicas/legales (la app interna NO se rastrea), hits solo en prod. **Banner de consentimiento propio opt-in RGPD** (`components/analytics/Analytics.tsx` + `CookieBanner.tsx`; gtag no carga hasta «Aceptar»; revocable desde el footer vía `CookiePrefs`/`lib/consent.ts`). CSP de `next.config.ts` permite los dominios de Google. Evento de conversión **`generate_lead`** en el envío del formulario (`lib/gtag.ts`), marcado como **evento clave** en GA4 el 2026-06-11. Vinculado a Search Console. *(Iubenda se probó y se revirtió el 2026-07-05, ver §3.)* |
