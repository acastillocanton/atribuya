...
| —. Google OAuth Business Profile (Vía B) — todas las reseñas | 🟡 **APROBADA POR GOOGLE 2026-07-10** (email "Your project has been approved to use the Google Business Profile API", proyecto `443155173600`), tras 3 solicitudes (rechazos 2026-06-24 por mismatch de dominio y 2026-06-30 por "internal quality checks"; corregidos: consent screen alineado con `castillocanton.com`, términos creados, `alejandro@atribuya.com` a buzón real + Propietario IAM). **Env configuradas en Vercel prod (2026-07-10)** vía API REST de Vercel (la CLI `env add` no aceptaba stdin en el entorno → guardaba vacías): `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` desde `.env.local` + `GOOGLE_OAUTH_REDIRECT_URI` = `https://atribuya.com/api/google/oauth/callback` (apex, sin www) + redeploy hecho. **PENDIENTE (próxima sesión, manual)**: (1) verificar OAuth E2E en `/fichas` (botón "Conectar Google" habilitado → conectar ficha → confirmar que trae TODAS las reseñas, no solo el top-5 de la Vía A); (2) para onboardear clientes externos, **publicar el consent screen** (scope `business.manage` es sensible → posible verificación de scopes; en modo Testing solo funcionan los test users); (3) confirmar cuota Business Information API ~300. Detalle → handoff §7.3. |
...
| **Google OAuth (Vía B)** | 🟡 **APROBADA POR GOOGLE 2026-07-10** (email de aprobación, proyecto `443155173600`) + **env ya en Vercel prod**: `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`GOOGLE_OAUTH_REDIRECT_URI` (=`https://atribuya.com/api/google/oauth/callback`, apex SIN www) subidas por API REST de Vercel el 2026-07-10 (la CLI `env add` no aceptaba stdin → las guardaba vacías) + redeploy. **Falta (manual, próxima sesión)**: (1) probar OAuth E2E en `/fichas` (conectar ficha → traer todas las reseñas); (2) **publicar el consent screen** para clientes externos (scope `business.manage` sensible; en Testing solo test users); (3) confirmar cuota Business Information API ~300. Detalle → handoff §7.3. |
...
En orden: ~~Brevo SMTP~~ (✅ hecho 2026-06-06) → ~~Google Cloud Places (Vía A)~~ (✅ hecho 2026-06-07) → ~~DPA~~ (✅ hecho 2026-06-07, ver fase 8) → **OAuth Business Profile (Vía B): ✅ APROBADA por Google 2026-07-10 + env ya en Vercel prod**. Solo falta la verificación E2E manual (conectar una ficha en `/fichas` y confirmar que trae todas las reseñas) y, para clientes externos, publicar el consent screen. Ver §7 y handoff §7.3. Con esto, **no queda ningún bloqueante técnico** para el primer cliente; lo demás de §8 son decisiones de negocio.
