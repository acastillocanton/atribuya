...
| —. **Mejoras SERP + blog bilingüe con Sanity CMS**: favicon válido para Google (ico 16/32/48 + icon 192px), titles ≤60, JSON-LD `WebSite`/`Organization`/`SoftwareApplication` en las homes; blog `/blog` (ES) + `/en/blog` (EN) con Studio embebido en `/studio` (sanity@4 + next-sanity@11, ⚠️ no subir a v5/v12 hasta Next 16), ISR 600s, modo degradado sin env vars, bugfix noindex/403 de `/en/terms`+`/en/privacy`. **Pendiente activación**: crear proyecto Sanity + env vars + CORS (pasos → handoff §21) | ✅ 2026-07-04 |
