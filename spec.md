...
4. ~~Pricing tiers exactos~~ — **decidido (2026-06-11, v3)**: modelo **híbrido por nº de comerciales con tope de fichas por plan**. Razón: casi todo el mercado objetivo tiene UNA ficha pero varios comerciales; el valor del producto crece con el equipo, no con las fichas (la v2 por fichas metía a todos en el tier barato sin ingreso de expansión). Las fichas quedan como segundo eje porque son lo que cuesta dinero en API. **3 tiers + a medida**: Básico (`basic`, hasta 5 comerciales + 1 ficha, 45 €/mes), Estándar (`standard`, hasta 15 comerciales + 3 fichas, 99 €/mes, **el destacado**), Plus (`plus`, hasta 30 comerciales + 10 fichas, 199 €/mes), A medida (`custom`, +30 / +10 fichas). **Los directores de oficina ocupan plaza de comercial** (son productores); cuentan los perfiles `role in (sales, office_director)` con `status in (invited, active)`, pausar libera plaza. Bloques holgados a propósito: mitigan el incentivo a no dar de alta a todo el equipo, que rompería la atribución (el matcher necesita el roster completo). Enforcement a nivel de app: `planLocationLimit()`/`planSalesLimit()` en `app/(super)/super/plans.ts` + `lib/plan-seats.ts`, impuesto en `createLocation`, `inviteSales`, `inviteOfficeDirector` y las reactivaciones. Los valores legacy v2 (`starter`/`professional`) conservan su tope de fichas y siguen con comerciales ilimitados. *Histórico v2 (2026-06-07): por nº de fichas, Starter 2 fichas 45 €, Professional 10 fichas 149 €, comerciales ilimitados.*
