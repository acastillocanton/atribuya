...
export type LocationCtx = {
  ...
  name: string;
  /**
   * Org the location belongs to (required for multi-tenant). Forwarded to
   * the `reviews` and `audit_log` INSERTs so admin/manager/sales of the
...
export type PendingNotification = {
  ...
  /** Org of the review — forwarded to audit_log rows when notif fails. */
  orgId: string;
};
...
export async function flushNotifications(
  ...
  notifyFn: (
    n: Omit<PendingNotification, "reviewDbId" | "salesId" | "googleReviewId" | "orgId"> & {
      appBase: string;
    },
  ) => Promise<
    | { ok: true }
