...
import type { MatchState } from "@/lib/supabase/types";

/**
 * Alerta temprana por reseña con rating bajo (≤2★) — admin + manager +
 * (si counted/pending) sales atribuido.
 *
 ...
 * (Verificación + Google).
 *
 * Multi-destinatario vía BCC para no exponer la lista de stakeholders unos a
 * otros (admin no debería ver el email del comercial en el "To"). `sendEmail`
 * de brevo.ts acepta `bcc`.
 */
...
/** URL pública de Google para ver/responder las reseñas de una ficha. */
function googleReviewListUrl(placeId: string | null): string | null {
...
export async function notifyLowRating(input: LowRatingNotificationInput) {
  ...
  // BCC para no exponer destinatarios entre sí. El "to" va al FROM (Brevo lo
  // permite — el remitente como receptor "fantasma").
  return sendEmail({
...
function renderText(
  ...
  return [
    `⚠️ Reseña con rating bajo recibida en Atribuya.`,
    "",
    ...
    <tr><td align="center" style="padding:48px 16px;">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="560" style="width:560px;max-width:100%;">
        ...
        <tr><td style="padding:24px 8px 0 8px;font-size:11.5px;line-height:1.6;color:#a8a294;">
          Recibes este correo porque tu rol (admin / gestor / comercial atribuido) recibe alertas por reseñas con rating bajo. Si quieres que dejen de llegar, contacta con el administrador.
          <br><br>
          Atribuya
        </td></tr>
      </table>
