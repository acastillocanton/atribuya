...
export async function syncPlaces(args: SyncPlacesArgs = {}): Promise<SyncPlacesResult> {
  ...
  // Multi-tenant: process only locations that belong to ACTIVE orgs. Orgs
  // in `trial` also count as active for the cron — they're paying customers
  // in a grace period. Locations of `suspended` or `churned` orgs are
  // skipped silently. If an org is misconfigured (e.g. status changed to
  // suspended mid-cron), the affected locations just don't appear in this
  // query — no need for per-org try/catch.
  const orgsRes = await admin
    ...
    .returns<{ id: string }[]>();
  if (orgsRes.error) {
    console.error("[sync-places] failed listing active orgs:", orgsRes.error);
    return {
  ...
  const activeOrgIds = (orgsRes.data ?? []).map((o) => o.id);
  ...
  const [locationsRes, salesRes, staffRes] = await Promise.all([
    locationsQuery.returns<
      { id: string; name: string; google_place_id: string; org_id: string }[]
    >(),
    admin
      .from("profiles")
  ...
  // cada alerta solo va a staff de la org dueña de la reseña).
  ...
  for (const loc of locations) orgByLocationId.set(loc.id, loc.org_id);
  ...
  // Alertas ≤2★: resolvemos nombres de cliente para los alerts atribuidos y
  // disparamos el flush (admin + manager por org + sales atribuido).
