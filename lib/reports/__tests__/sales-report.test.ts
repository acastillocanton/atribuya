...
describe("formatZoneForExcel", () => {
  it("devuelve el nombre de la ficha si existe", () => {
    expect(formatZoneForExcel("Acme Promotora · Centro")).toBe(
      "Acme Promotora · Centro",
    );
  });

  it("devuelve '—' si no hay ficha", () => {
    expect(formatZoneForExcel(null)).toBe("—");
  });
});
